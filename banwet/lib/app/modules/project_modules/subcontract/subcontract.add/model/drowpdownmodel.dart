...
//
//     final selectedDropdowndataModel = selectedDropdowndataModelFromJson(jsonString);

import 'dart:convert';

SelectedDropdowndataModel selectedDropdowndataModelFromJson(String str) =>
    SelectedDropdowndataModel.fromJson(json.decode(str));

String selectedDropdowndataModelToJson(SelectedDropdowndataModel data) =>
    json.encode(data.toJson());

class SelectedDropdowndataModel {
  final bool status;
  ...
  factory SelectedDropdowndataModel.fromJson(Map<String, dynamic> json) =>
      SelectedDropdowndataModel(
        status: json["status"],
        accountHeads:
            List<Head>.from(json["account_heads"].map((x) => Head.fromJson(x))),
        expenseHeads:
            List<Head>.from(json["expense_heads"].map((x) => Head.fromJson(x))),
        paymentModes: List<PaymentMode>.from(
            json["payment_modes"].map((x) => PaymentMode.fromJson(x))),
        projects: List<Project>.from(
            json["projects"].map((x) => Project.fromJson(x))),
        labours:
            List<Labour>.from(json["labours"].map((x) => Labour.fromJson(x))),
        workTypes:
            List<Type>.from(json["work_types"].map((x) => Type.fromJson(x))),
        measureUnits: List<MeasureUnit>.from(
            json["measure_units"].map((x) => MeasureUnit.fromJson(x))),
        toolCategories: List<ToolCategory>.from(
            json["tool_categories"].map((x) => ToolCategory.fromJson(x))),
        taxPercentagesNew: List<TaxPercentagesNew>.from(
            json["tax_percentages_new"]
                .map((x) => TaxPercentagesNew.fromJson(x))),
        taxPercentages:
            List<String>.from(json["tax_percentages"].map((x) => x)),
        taVehicleType: List<TaVehicleType>.from(
            json["ta_vehicle_type"].map((x) => TaVehicleType.fromJson(x))),
        contractors: List<Contractor>.from(
            json["contractors"].map((x) => Contractor.fromJson(x))),
        vehicleDocumentTypes: List<Type>.from(
            json["vehicle_document_types"].map((x) => Type.fromJson(x))),
        ...
        taxStateCodes: List<TaxStateCode>.from(
            json["tax_state_codes"].map((x) => TaxStateCode.fromJson(x))),
        customers: List<Map<String, String?>>.from(json["customers"].map(
            (x) => Map.from(x).map((k, v) => MapEntry<String, String?>(k, v)))),
        salePriceCategories: List<SalePriceCategory>.from(
            json["sale_price_categories"]
                .map((x) => SalePriceCategory.fromJson(x))),
      );

  Map<String, dynamic> toJson() => {
        "status": status,
        "account_heads":
            List<dynamic>.from(accountHeads.map((x) => x.toJson())),
        "expense_heads":
            List<dynamic>.from(expenseHeads.map((x) => x.toJson())),
        "payment_modes":
        ...
        "tool_categories":
            List<dynamic>.from(toolCategories.map((x) => x.toJson())),
        "tax_percentages_new":
            List<dynamic>.from(taxPercentagesNew.map((x) => x.toJson())),
        "tax_percentages": List<dynamic>.from(taxPercentages.map((x) => x)),
        "ta_vehicle_type":
            List<dynamic>.from(taVehicleType.map((x) => x.toJson())),
        "contractors": List<dynamic>.from(contractors.map((x) => x.toJson())),
        "vehicle_document_types":
            List<dynamic>.from(vehicleDocumentTypes.map((x) => x.toJson())),
        "supervisors": List<dynamic>.from(supervisors.map((x) => x.toJson())),
        "bank_accounts":
            List<dynamic>.from(bankAccounts.map((x) => x.toJson())),
        "services": List<dynamic>.from(services.map((x) => x.toJson())),
        "tax_state_codes":
            List<dynamic>.from(taxStateCodes.map((x) => x.toJson())),
        "customers": List<dynamic>.from(customers.map(
            (x) => Map.from(x).map((k, v) => MapEntry<String, dynamic>(k, v)))),
        "sale_price_categories":
            List<dynamic>.from(salePriceCategories.map((x) => x.toJson())),
      };
}
...
class SalePriceCategory {
  ...
  factory SalePriceCategory.fromJson(Map<String, dynamic> json) =>
      SalePriceCategory(
        value: json["value"],
        name: json["name"],
      );
...
class TaxPercentagesNew {
  ...
  factory TaxPercentagesNew.fromJson(Map<String, dynamic> json) =>
      TaxPercentagesNew(
        taxId: json["tax_id"],
